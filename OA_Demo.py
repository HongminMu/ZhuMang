# 处理普通摄像头：
# --trained_model=weights/yolact_coco_custom_55_1666.pth --config=yolact_coco_custom_config --score_threshold=0.35 --top_k=6 --video=0 --display
# 本代码实现视觉避障-语音提醒
# 由于laptop不接电时GPU处理帧率过低，5帧每秒读入，每一帧检测到就输出语音
# 背带摄像头应该45度倾斜向下，只留一米盲区，若倾角过小检测不到近处
# 🚀🚀🚀🚀🚀🚀🚀🚀🚀
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    flag = 0
    # print('num_dets_to_consider:')  # len(class)有几个非背景类东西
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        for ID in range(len(classes)):
            ...
            if classes[ID] == 3:  # ground对应 ID=2  0,1,2,3
                flag = 1
                # print('ID:  ')
                # print(ID)
                # print("XX.shape:    ")
        ...
        if flag == 1:
            ...
            def check_ok(X, Y):
                ...
                for i in range(x1, mid_X, 3):   # 左区域w:0.35-0.5
                    for j in range(50, dety, 3):
                        if XX_numpy[Y - j, i, 0] == 0:
                            count1 += 1
                            XX_numpy[Y - j, i, 0] = 0  # BGR格式 R   #(0， 215， 255)金黄色
                            XX_numpy[Y - j, i, 1] = 0
                ...
                #     for j in range(dety, Y, 8):
                for i in range(mid_X, x2, 3):  # 右区域w:0.5-0.65
                    for j in range(50, dety, 3):
                        if XX_numpy[Y - j, i, 0] == 0:
                            count2 += 1
                            XX_numpy[Y - j, i, 0] = 0  # BGR格式 R   #(0， 215， 255)金黄色
                ...
                #  draw the far area box (have not been used)
                t0 = -1
                for j in range(dety + 20, Y - 50, 3):
                    t0 += 1
                    t = int(t0*0.2)
                    for i in range(x1+t, x2-t, 3):
                        if XX_numpy[Y - j, i, 0] == 0:
                            count = count + 1
                            XX_numpy[Y - j, i, 0] = 0  # BGR格式 R   #(0， 215， 255)金黄色
                            XX_numpy[Y - j, i, 1] = 0
                            ...
                            XX_numpy[Y - j, i, 0] = 0  # BGR格式 B   #(0， 215， 255)金黄色
        ...
        while vid.isOpened() and running:
            ...
            if not (vid_done and len(active_frames) == 0):
                ...
                for frame in active_frames:
                    ...
                    # 改成阻塞式：
                    # frame['value'] = pool.apply(sequence[frame['idx']], args=_args)
